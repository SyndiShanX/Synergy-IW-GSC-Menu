  ...
  for (;;) {
      ...
      if(!var_01) {
        if(!var_04) {
          ...
          if(soundexists("antigrav_battery_damaged_warning_lp")) {
            thread scripts\sp\_utility::play_loop_sound_on_tag("antigrav_battery_damaged_warning_lp", "tag_origin", 1, 1);
          }
        }
    ...
    if(!var_03) {
      ...
      if(soundexists("antigrav_battery_damaged_lp")) {
        var_14 thread scripts\sp\_utility::play_loop_sound_on_tag("antigrav_battery_damaged_lp", "tag_origin", 1, 1);
      }
    }
