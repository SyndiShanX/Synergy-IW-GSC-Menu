...
_meth_8315() {
  ...
  if(scripts\sp\_utility::func_93A6() && !scripts\engine\utility::array_contains(var_01, "specialty_extraequipment")) {
  ...
  for (;;) {
    ...
    if(level.player getteamsize()) {
      ...
      if(var_02 != "") {
        ...
        var_06 scripts\engine\utility::delaythread(60, ::scripts\sp\_utility::func_F1DE);
      }
    }
...
func_13C35() {
  ...
  while (var_05.size > 0) {
    ...
    if(!scripts\sp\_utility::array_compare(var_0A, var_09)) {
...
func_3D6E() {
  ...
  if(var_01 > 0) {
    scripts\sp\_utility::settimer("FIRST_EQUIP_UPGRADE");
  }

  if(var_01 == var_00) {
    scripts\sp\_utility::settimer("ALL_EQUIP_UPGRADES");
...
func_3DAE() {
  ...
  if(var_00 > 0) {
    scripts\sp\_utility::settimer("FIRST_JACKAL_ITEM");
  }

  if(var_00 == var_01) {
    scripts\sp\_utility::settimer("ALL_JACKAL_ITEMS");
  }
}
...
func_3DDF(param_00) {
  ...
  if(var_01[0] > 0) {
    scripts\sp\_utility::settimer("SCAN_1_WEAPON");
  }

  if(var_01[0] >= 10) {
    scripts\sp\_utility::settimer("SCAN_10_WEAPONS");
  }

  if(var_01[0] == var_01[1]) {
    scripts\sp\_utility::settimer("SCAN_ALL_WEAPONS");
...
func_3DDD() {
  ...
  if(var_01 > 0) {
    scripts\sp\_utility::settimer("FIRST_WANTED_BOARD");
  }

  if(var_01 > 15) {
    scripts\sp\_utility::settimer("HALF_WANTED_BOARD");
  }

  if(var_01 == var_02) {
    scripts\sp\_utility::settimer("ALL_WANTED_BOARD");
...
func_3D6A(param_00) {
  ...
  if(var_02) {
    scripts\sp\_utility::settimer("DOOR_PEEK");
    if(isdefined(param_00) && param_00) {
...
func_3D61() {
  ...
  for (;;) {
    ...
    if(level.player gold_teeth_hint_func()) {
      ...
      var_02 = 0;
      var_03 = 0;
    } else if(level.player scripts\engine\utility::get_doublejumpenergy() >= 390 && !level.player isjumping() && var_06 > 1.5 && !scripts\sp\_utility::func_93AC() && !level.player islinked()) {
      if(var_02 == 0) {
        var_02 = gettime() / 1000;
      }
    ...
    if(level.var_C538 >= var_01) {
      scripts\sp\_utility::settimer("NO_JUMPING");
...
func_5F2F() {
  for (var_00 = 0; var_00 < level.var_B8D2.var_ABFA.size; var_00++) {
    ...
    var_02 = level.player _meth_84C6("missionStateData", scripts\sp\_endmission::func_7F6D(var_00));
    if(var_01 == 1) {
