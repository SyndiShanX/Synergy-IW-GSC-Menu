...
func_D45C() {
  ...
  while (isalive(level.player) && scripts\anim\battlechatter::func_29CA() && isdefined(level.player.var_28CF) && level.player.var_28CF) {
    if(!scripts\engine\utility::player_is_in_jackal()) {
      var_01 = scripts\sp\_utility::maymovefrompointtopoint();
      foreach(var_03 in var_01) {
        ...
        if(issubstr(var_03.classname, "dropship")) {
          if(isdefined(var_03.script_team) && var_03.script_team != level.player.team) {
...
func_9FE0(param_00) {
  if(!isdefined(self.var_10AC8.var_9E9B[param_00]) || !isdefined(level.isteamsaying[self.team][param_00])) {
    return 1;
  }

  if(!self.var_10AC8.var_9E9B[param_00] && !level.isteamsaying[self.team][param_00]) {
    return 1;
  }
