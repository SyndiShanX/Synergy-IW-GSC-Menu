...
func_F159() {
  ...
  func_F13A();
  thread scripts\sp\_utility::play_sound_on_entity("seeker_init");
...
func_F11E(param_00, param_01, param_02) {
  if(isdefined(self)) {
    ...
    if(isdefined(self.triggerportableradarping) && level.player == self.triggerportableradarping) {
      var_04 = self.triggerportableradarping scripts\sp\_detonategrenades::func_734E();
      self.triggerportableradarping scripts\engine\utility::delaythread(0.1, ::scripts\sp\_detonategrenades::func_734D, var_03, var_04, level.var_F10A.var_4D19);
    }
...
func_F110(param_00, param_01) {
  if(level.player scripts\sp\_utility::func_65DB("no_grenade_block_gesture") || level.player isthrowinggrenade() || level.player gold_teeth_pickup_debug()) {
    return;
  }
...
func_7C41(param_00) {
  ...
  if(scripts\engine\utility::flag_exist("stealth_enabled") && scripts\engine\utility::flag("stealth_enabled")) {
    if(isalive(self.triggerportableradarping) && self.triggerportableradarping scripts\sp\_utility::func_65DF("stealth_attack") && !self.triggerportableradarping scripts\sp\_utility::func_65DB("stealth_attack")) {
      return undefined;
    }
  }
