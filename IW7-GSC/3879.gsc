...
func_79F5(param_00) {
  ...
  if(level.var_10E6D.group.groups[param_00].size) {
    level.var_10E6D.group.groups[param_00] = scripts\sp\_utility::func_22B9(level.var_10E6D.group.groups[param_00]);
  }
...
func_868B(param_00) {
  if(isdefined(self.var_EED1)) {
    self.var_EED1 = scripts\sp\_utility::string(self.var_EED1);
  } else {
...
func_CE43(param_00) {
  ...
  if(soundexists(param_00)) {
    ...
    if(isdefined(self.stealth_vo_ent)) {
      if(isdefined(self.model) && scripts\sp\_utility::hastag(self.model, "j_head")) {
