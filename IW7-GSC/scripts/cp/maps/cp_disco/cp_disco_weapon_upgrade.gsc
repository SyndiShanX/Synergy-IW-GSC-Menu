 ...
 * Script: scripts\cp\maps\cp_disco\cp_disco_weapon_upgrade.gsc
****************************************************************/
...
weapon_upgrade(param_00, param_01) {
  ...
  if(var_03 can_upgrade(var_04)) {
    ...
    if(var_03 scripts\cp\utility::is_valid_player()) {
      ...
      var_03 scripts\cp\cp_merits::processmerit("mt_dlc2_upgrade_weapons");
    }
...
wait_for_player_to_take_weapon(param_00, param_01, param_02) {
  ...
  for (;;) {
    ...
    if(scripts\engine\utility::istrue(var_03.kung_fu_mode)) {
      wait(0.1);
      continue;
    }
